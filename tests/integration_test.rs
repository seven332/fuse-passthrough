...
use common::{
    setup_test_dirs, wait_for, wait_for_dir, wait_for_file, wait_for_file_gone, MountGuard,
};
...
fn test_write_file() {
    ...
    assert!(
        wait_for_file(&source.join("new.txt")),
        "File not created in source"
    );
...
fn test_create_directory() {
    ...
    assert!(
        wait_for_dir(&source.join("newdir")),
        "Directory not created in source"
    );
}
...
fn test_delete_file() {
    ...
    assert!(
        wait_for_file_gone(&source.join("to_delete.txt")),
        "File still exists in source"
    );
}
...
fn test_rename_file() {
    ...
    assert!(
        wait_for_file(&source.join("renamed.txt")),
        "Renamed file does not exist"
    );
    assert!(
        wait_for_file_gone(&source.join("original.txt")),
        "Original file still exists"
    );

    let content =
        fs::read_to_string(source.join("renamed.txt")).expect("Failed to read renamed file");
    assert_eq!(content, test_content);
}
...
fn test_delete_directory() {
    ...
    assert!(
        wait_for_file_gone(&source.join("to_delete_dir")),
        "Directory still exists in source"
    );
}
...
fn test_symlink() {
    ...
    let content =
        fs::read_to_string(mountpoint.join("link.txt")).expect("Failed to read through symlink");
    assert_eq!(content, test_content);
}
...
fn test_rename_across_directories() {
    ...
    assert!(
        wait_for_file(&source.join("dir2/file.txt")),
        "File not found in dir2"
    );
    assert!(
        wait_for_file_gone(&source.join("dir1/file.txt")),
        "File still exists in dir1"
    );
...
fn test_nested_directories() {
    ...
    assert!(
        wait_for_file(&source.join("a/b/c/file.txt")),
        "Nested file not created"
    );
    assert!(
        source.join("a/b/c").is_dir(),
        "Nested directories not created"
    );
...
fn test_file_metadata() {
    ...
    let source_metadata =
        fs::metadata(source.join("metadata.txt")).expect("Failed to get metadata from source");

    // Verify metadata matches
    assert_eq!(mount_metadata.len(), source_metadata.len(), "Size mismatch");
    assert_eq!(
        mount_metadata.is_file(),
